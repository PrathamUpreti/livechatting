...
export default function ChatWidget({
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      setConnected(true);
      const savedPasscode = localStorage.getItem("chatwidget_passcode");
      if (savedPasscode) {
        socket.emit("join_room", { roomId, name, role, passcode: savedPasscode });
      }
    });
    ...
    socket.on("join_result", ({ ok, reason }) => {
      ...
      if (ok) {
        ...
        localStorage.removeItem("chatwidget_passcode");
      }
    });
    ...
    socket.on("receive_message", (message) => {
      ...
      setOpen((currentlyOpen) => {
        if (!currentlyOpen && message.senderId !== socket.id) {
          setUnread((u) => u + 1);
        }
  ...
  const submitPasscode = (e) => {
    ...
    setPasscodeError("");
    socketRef.current?.emit("join_room", { roomId, name, role, passcode: code });
    localStorage.setItem("chatwidget_passcode", code);
  };
          ...
          {authorized ? (
                ...
                {messages.map((m, i) =>
                  m.system ? (
                    ...
                    <MessageBubble
                      key={m.id}
                      message={m}
                      isOwn={m.senderId === socketRef.current?.id}
                      showName={
            ...
            >
              ...
              <p className="text-[12.5px] text-wa-sub">
                Ask whoever shared this with you for the passcode.
              </p>
              <input
                type="password"
                value={passcodeInput}
