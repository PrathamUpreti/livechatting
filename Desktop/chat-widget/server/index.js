...
io.on("connection", (socket) => {
  ...
  socket.on("join_room", ({ roomId, name, role, passcode, clientId }) => {
    ...
    // Prefer the browser's persistent clientId (survives reconnects) so a
    // person's own messages don't flip to the "received" side after they
    // reload or briefly drop connection. Falls back to socket.id for any
    // older client that doesn't send one.
